...
SELECT d.product_id, IFNULL(p.new_price, 10) AS price
FROM distinct_id d
    LEFT JOIN target_price t ON t.product_id = d.product_id
    LEFT JOIN Products p ON p.product_id = t.product_id AND p.change_date = t.change_date
;
